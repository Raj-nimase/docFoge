    ...
    <main className="editor-panel">
      <div className="editor-panel-topbar">
        <div className="editor-panel-breadcrumb">
          <span className="breadcrumb-project">{currentProject?.metadata?.title || 'Project'}</span>
          <span className="breadcrumb-sep">›</span>
          <span className="breadcrumb-chapter">{section?.label || section?.title || '—'}</span>
        </div>
        <MetadataForm />
      </div>
      ...
      {isAutoSection ? (
        <div className="auto-section-notice">
          ...
          <div className="auto-section-text">
            <strong>{section.label}</strong> is generated automatically from your document settings.
          </div>
        ...
        <ChapterEditor
          ...
          onContentChange={json => updateSectionContent(activeChapterId, json)}
        />
      ) : (
        <div className="editor-empty-state">
          <div className="editor-empty-icon">📝</div>
          <div className="editor-empty-text">Select a chapter from the left panel to start editing</div>
        </div>
      )}
    </main>
  );
